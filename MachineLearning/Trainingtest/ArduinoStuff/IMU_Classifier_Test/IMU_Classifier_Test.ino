...
//----------------------------------
String message[NUM_GESTURES];
double werte[NUM_GESTURES];
//---------------------------------
void setup() {
  Serial.begin(9600);
  while (!Serial);
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (samplesRead == numSamples) {
        ...
        for (int i = 0; i < NUM_GESTURES; i++) {
          ...
          message[i] = String(GESTURES[i]) + ":" + String(tflOutputTensor->data.f[i]);
          werte[i] = tflOutputTensor->data.f[i];
...
//https://forum.arduino.cc/t/returning-max-value-from-array/42111/5
